...
export async function fetchArticleFromUrl(url, signal) {
  ...
  for (const proxy of PROXIES) {
    try {
      const res = await fetch(proxy.buildUrl(normalized), { signal })
      if (!res.ok) {
      ...
      if (!article || !article.textContent?.trim()) {
        throw new Error('无法提取文章正文。该页面可能需要登录、有付费墙或反爬限制，请切换到「手动粘贴」模式')
      }
      return {
        title: article.title?.trim() || new URL(normalized).hostname,
  ...
  if (lastErr?.name === 'TypeError' || msg.toLowerCase().includes('failed to fetch')) {
    throw new Error('网络连接失败，无法访问代理服务。请检查网络连接后重试，或切换到「手动粘贴」模式')
  }
  throw new Error(msg || '导入失败，请切换到「手动粘贴」模式')
}
